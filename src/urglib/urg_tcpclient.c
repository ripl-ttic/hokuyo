...
/* #endif */
    ...
    if ((cli->sock_desc = (int)socket(AF_INET, SOCK_STREAM, 0)) < 0) {
        fprintf (stderr, "tcpclient_open: Error calling socket. errno = %s\n",
                 strerror (errno));
        return -1;
    }
    ...
    /*     } */
    /* } */
    /* //ブロックモードにする [Block mode to] */
    /* set_block_mode(cli); */

#else
    // ノンブロックに変更 [Change in non-blocking]
    ...
    if (connect(cli->sock_desc, (const struct sockaddr *)&(cli->server_addr),
        ...
        if (errno != EINPROGRESS) {
            fprintf (stdout, "tcpclient_connect: closing due to error. connect reports %s\n",
                     strerror (errno));
            tcpclient_close(cli);
            return -1;
        }
